	...
	$.ajax({
        ...
        success:function(data){
	        $.each(data, function (i, item) {
	        	if(item.custId == custSell){
	        		$("#custSell").append("<option value='" + item.custId + "' selected>" + item.custName + "</option>");
	        	}else{
...
function confirmForm() {
    $("#factorAmdForm").validate({
      submitHandler : function() {  //验证通过后的执行方法
          //当前的form通过ajax方式提交（用到jQuery.Form文件）
      	var param = $("#factorAmdForm").serialize();
          $.ajax({
              url : ctx+"/factorController/factorAmdConfirm",
              type : "post",
              dataType : "json",
              data: param,
              success:function(data){
                  if(data.isSuc){
                	  document.getElementById("factorAmdForm").reset(); 
                      swal({title: '成功',
                		  text: data.errMsg,
                		  type: 'success',
