...
public class CustomerInfoDaoImpl<CustomerInfo> extends BaseDaoImpl<CustomerInfo> implements CustomerInfoDao<CustomerInfo>{

	@SuppressWarnings("unchecked")
	@Override
	public List<Map<String, Object>> getComboList() {
		String sql = "select * from customer_info where status > 0 ";
	...
	public List<Map<String, Object>> getList(Integer pageNumber,Integer pageSize,Map<String, Object> mapData,boolean isPage) {
		String sql ="select "
					...
					+ "checkId, "
					+ "status, "
					+ "(select u.userName from sys_user u where u.userId = customer_info.modOptr) as modOptr, "
					+ "modTime "
				+ "from customer_info "
				+ "where status > 0 ";
		...
		if (isPage) {
			sql += " order by status ,"
				+  " custId limit " + (pageNumber - 1) * pageSize + "," + pageSize;
		}
